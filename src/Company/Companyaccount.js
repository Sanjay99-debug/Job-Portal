...
function Companyaccount() {
  ...
  const fetchCompanyData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/api/Companyjpcs/by-employee/${employeeId}`
      );
  ...
  const handleSave = async () => {
    try {
      if (company) {
        // Update
        await axios.put(
          `${process.env.REACT_APP_API_BASE_URL}/api/Companyjpcs/${employeeId}`,
          {
            ...formData,
            employeeId,
          }
        ...
        // Add
        await axios.post(
          `${process.env.REACT_APP_API_BASE_URL}/api/Companyjpcs`,
          {
            ...formData,
            employeeId,
          }
  ...
  const handleExport = () => {
    const header =
      "ID,Company Name,Address,City,Pincode,Mobile,Email,Contact Person,Detail\n";
    const data = company
  ...
  return (
    <div
      ...
      style={{ background: "linear-gradient(to right, #90bae7ff, #2a9ae9ff)" }}
    >
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h3>
          <i className="bi bi-building text-success me-2"></i>Company Account
        </h3>
      ...
      <div className="card p-4 shadow-lg rounded-4 border-0">
        {company ? (
          <div className="row">
            ...
            <div className="col-md-6">
              <strong>Mobile No.:</strong> {company.mobile}
            </div>
            <div className="col-md-6">
            ...
            <div className="col-12 mt-2">
              <strong>Detail:</strong> {company.detail}
            </div>
          </div>
        ) : (
              ...
              <div className="modal-content">
                <div className="modal-header">
                  <h5 className="modal-title">
                    {company ? "Update Company Details" : "Add Company Details"}
                  </h5>
                  <button
                    type="button"
                    className="btn-close"
